...
cd $BUILD_DIR/otcl*
OTCL_DIR=`pwd`
./configure --with-tcl=$TCL_DIR --x-includes=$XORG_DIR --prefix=/usr/local
make
sudo make install

# this is for the run script
sudo ln -s $OTCL_DIR/libotcl.so /usr/local/lib/libotcl.so
LD_LIBRARY_PATH=$OTCL_DIR:/usr/lib/:/usr/local/lib/:$LD_LIBRARY_PATH
...
cd $BUILD_DIR/tclcl*
TCLCL_DIR=`pwd`
./configure --x-includes=$XORG_DIR --with-tcl=$TCL_DIR
make
sudo make install
...
cd $BUILD_DIR/ns-*
NS2_DIR=`pwd`
./configure --with-tclcl=$TCLCL_DIR --with-otcl=$OTCL_DIR --x-libraries=$OTCL_DIR --x-includes=$XORG_DIR --with-tcl=$TCL_DIR
...
./configure --with-tclcl=$TCLCL_DIR --with-otcl=$OTCL_DIR --x-libraries=$OTCL_DIR \
    --x-includes=$XORG_DIR --with-tcl=$TCL_DIR --with-zlib=$ZLIB_DIR
